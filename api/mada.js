...
exports.getMada = void 0;
var O = require("fp-ts/Option");
var function_1 = require("fp-ts/function");
var madaRequest = function () {
    return new Request('https://www.mdais.org/umbraco/api/invoker/execute', {
        headers: {
...
function getFileName() {
    var now = new Date();
    return "".concat(now.getDate(), ".").concat((now.getUTCMonth() + 1), ".").concat(now.getFullYear(), ".json");
}
function fetchFunction(req) {
    return (0, function_1.pipe)(O.tryCatch(function () { return fetch(req); }), O.chain(function (response) { return O.tryCatch(function () { return response.json(); }); }), O.map(function (response) { return response.Result; }));
}
exports.getMada = {
    fetchFunction: function () { return fetchFunction(madaRequest()); },
