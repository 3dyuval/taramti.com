...
const madaRequest = () =>
  new Request('https://www.mdais.org/umbraco/api/invoker/execute', {
    headers: {
...
function getFileName(): string {
    const now = new Date()
    return `${now.getDate()}.${(now.getUTCMonth() + 1)}.${now.getFullYear()}.json`
    }

function fetchFunction(req: Request) {
  return pipe(
    O.tryCatch(() => fetch(req)),
    O.chain((response) => O.tryCatch(() => response.json())),
    O.map((response) => response.Result as Row[])
  )
}

export const getMada: Getter<Row[]> = {
    fetchFunction: () => fetchFunction(madaRequest()),
    getFileName
};
